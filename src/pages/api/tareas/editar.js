...
export async function PUT({ request, cookies }) {
  ...
  const { id, title, description = '', status, asignado_a = null } = body;

  if (!id) {
    return new Response(JSON.stringify({ error: 'ID de tarea obligatorio' }), { status: 400 });
  }
  ...
  try {
    ...
    const result = await query(
      `UPDATE tareas
       SET titulo = $1, descripcion = $2, estado = $3, asignado_a = $4
       WHERE id = $5
       RETURNING id, titulo, descripcion, estado, asignado_a, created_at`,
      [title.trim(), description.trim(), estadoLimpio, asignado_a || null, id]
    );
